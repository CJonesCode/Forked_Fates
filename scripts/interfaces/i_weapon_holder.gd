...
virtual func get_weapon_hold_position() -> Vector2:
...
virtual func get_velocity() -> Vector2:
	assert(false, "get_velocity() must be implemented by IWeaponHolder")
	return Vector2.ZERO

## Check if the holder is in a valid state to hold weapons
virtual func can_hold_weapons() -> bool:
